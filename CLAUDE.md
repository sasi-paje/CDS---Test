...
- `<html>` must always carry `data-brand="orange"` `data-style="standard"` `data-theme="light"`
- Default component size is `md` — override only when context requires (table headers → `xs`/`sm`, landing CTAs → `lg`)
...
- Font: Inter (set by style preset)