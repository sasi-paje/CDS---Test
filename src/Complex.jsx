...
function Chart({ type = "line", data, x = "x", y = "y", height = 200, color, formatY, label, showGrid = true, showAxis = true }) {
  ...
  const colors = ["var(--bg-brand)", "var(--emerald-500)", "var(--amber-500)", "var(--rose-500)"];
    ...
    <div ref={ref} style={{ width: "100%" }}>
      ...
      <svg width={W} height={H} role="img">
        ...
        {showAxis && xs.map((xv, i) => {
          // sparse x labels — show first, last, middle
        ...
        {series.map((s, si) => {
          const c = color || colors[si % colors.length];
          if (type === "bar") {
          ...
          return (
            <g key={si}>
              ...
              {data.map((d, i) => (
                <circle key={i} cx={pad.l + i * xStep} cy={yScale(+d[s] || 0)} r="3" fill={c} />
              ))}
            </g>
          );
        })}
